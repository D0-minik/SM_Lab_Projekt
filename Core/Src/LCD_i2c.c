...
void lcd_init (LCD_HandleTypeDef* hLCD)
{
  ...
  // dislay initialisation
	lcd_send_cmd (hLCD,0x28); // Function set --> DL=0 (4 bit mode), N = 1 (2 line display) F = 0 (5x8 characters)
	HAL_Delay(1);
	lcd_send_cmd (hLCD,0x08); //Display on/off control --> D=0,C=0, B=0  ---> display off
	HAL_Delay(1);
	lcd_send_cmd (hLCD,0x01);  // clear display
	HAL_Delay(1);
	HAL_Delay(1);
	lcd_send_cmd (hLCD,0x06); //Entry mode set --> I/D = 1 (increment cursor) & S = 0 (no shift)
	HAL_Delay(1);
	lcd_send_cmd (hLCD,0x0C); //Display on/off control --> D = 1, C and B = 0. (Cursor and blink, last two bits)
}
